...
<button type="button" class="close" onclick="closeForm()">&times;</button>
        <input type="hidden" id="entrepreneur_id" name="entrepreneur_id" value="<?= $product[0]['entrepreneur_id']; ?>">
        <h4><i class="fa fa-store"></i>  <?=$product[0]['store_name']?></h4>
        <div class="card-body p-4" style="height: 400px; width: 350px; overflow-y: scroll; scroll-behavior: smooth; scroll-padding-bottom: 0.5em;">
...
?>
        ...
        <div class="card-footer">
            <div class="row">
                ...
                <div class="col-auto">
                    <button type="button" class="btn btn-submit pull-rigth" id="send-button">ส่ง</button>
                </div>
            </div>
        </div>
    </form>
...
$(document).ready(function() {
  ...
  setInterval(function() {
      $.ajax({
        ...
        data: { entrepreneur_id: $('#entrepreneur_id').val() },
        success: function(result) {
            // get the current scroll position
            var scrollTop = $("#chat .card-body").scrollTop();

            // update the chat box with the new messages
            $("#chat .card-body").html(result);

            // set the scroll position back to where it was before updating the chat box
            $("#chat .card-body").scrollTop(scrollTop);
        }
      });
