    ...
    <div class="container-fluid">
      ...
      <div class="card shadow mb-4">
        <div class="card-header py-3">
          <h6 class="m-0 font-weight-bold text-primary">แก้ไขหมวดหมู่สินค้า</h6>
        </div>
        <div class="card-body">
          <div class="card-body">
            <div class="row">
              <div class="col-sm-12">
                <form action="<?= base_url()?>store/category/edit" enctype="multipart/form-data" method="post">
								<div class="form-group">
                    <label  class="">ชื่อหมวดหมู่สินค้า</label>
                    <input type="hidden" name="category_id" value="<?= $category[0]['category_id']; ?>">
                    <input type="text" class="form-control" name="category_name" required="" placeholder="ชื่อหมวดหมู่สินค้า" value="<?= $category[0]['category_name']; ?>">
                    <?= form_error('name'),'<small class="text-danger pl-3">','</small>'; ?>
                  </div>
									<div class="form-group">
                    <label  class="">รายละเอียดสินค้า</label>
                    <textarea type="text" class="form-control" name="category_description" required="" placeholder="รายละเอียด" ><?= $category[0]['category_description']; ?></textarea>
                    <?= form_error('name'),'<small class="text-danger pl-3">','</small>'; ?>                 
