...
use Mike42\Escpos\PrintConnectors\WindowsPrintConnector;
    ...
    $printer -> feed(1);      
        ...
        $printer -> text($print[0]['name_temp']);
        $printer -> feed();
        $printer -> text("--------------------------------\n"); 
        /* Title of receipt */
        ...
        $printer -> feed();
        $printer -> setJustification(Printer::JUSTIFY_LEFT);
        $printer -> text($i['id_ticket']);
        $printer -> feed();
        $printer -> text($i['id_timetable']   );
        $printer -> text("      ");
        $printer -> text($i['price_ticket']);
        // $printer -> text("      ");        
        ...
        $printer -> setEmphasis(true);
        $printer -> text("Total     :              ");
        if (count($print) == '2') { $total = $print[0]['price_ticket'] + $print[0]['price_ticket'] ;;
                                            }else{ $total = $print[0]['price_ticket'] ;}
        $printer -> text($total);
        $printer -> setEmphasis(false);
        $printer -> feed();
        $printer -> text("--------------------------------\n");
        ...
        $printer -> close();
    $printer -> close();
        redirect('ticket/ticketsaya/'.$print[0]['id_customer']);